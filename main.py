...
play_again_image = pygame.transform.scale(play_again_image, (50, 100))  # Resize if needed
...
class Button:
    def __init__(self, image, pos, text_input=None, font=None, base_color=None, hovering_color=None):
        ...
        if self.image is not None:
            ...
            # If no image is provided, create a rect based on text size
            if self.text_rect:
                self.rect = self.text_rect
            else:
                # Default rect size if no image or text is provided
                self.rect = pygame.Rect(self.x_pos - 50, self.y_pos - 25, 100, 50)
...
def main():
    ...
    controller = Controller(WIN, WIDTH, HEIGHT)
    # Call your mainloop
    controller.mainloop()
